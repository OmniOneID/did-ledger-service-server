...
public class DidQueryServiceImpl implements DidQueryService {
    ...
    @Override
    public Optional<DidDocument> didDocRevokedFindLatestByDidIdAndVersion(Long didId, Short version) {
        return didDocumentRevokedRepository.findFirstByDidIdAndVersionOrderByRevokedAtDesc(didId, version)
                .map(revokedDoc -> {
                    DidDocument doc = new DidDocument();
                    doc.setId(revokedDoc.getId());
                    doc.setVersion(revokedDoc.getVersion());
                    doc.setDocument(revokedDoc.getDocument());
                    doc.setController(revokedDoc.getController());
                    doc.setDeactivated(revokedDoc.getDeactivated());
                    doc.setDidId(revokedDoc.getDidId());
                    doc.setCreatedAt(revokedDoc.getCreatedAt());
                    doc.setUpdatedAt(revokedDoc.getUpdatedAt());
                    return doc;
                });
    }
    ...
    public void revokeDidDocument(Long didId) {
        ...
        List<DidDocumentRevoked> didDocumentRevokedList = didDocumentList.stream().map(didDocument -> {
            DidDocumentRevoked revokedDoc = DidDocumentRevoked.builder()
                    ...
                    .build();
            // Preserve original creation time from did_document table
            revokedDoc.setCreatedAt(didDocument.getCreatedAt());
            return revokedDoc;
        ...
        List<DidDocumentStatusHistory> revokedDidDocumentList = didDocumentList.stream().map(didDocument -> DidDocumentStatusHistory.builder()
                        .version(didDocument.getVersion())
                        .didId(didDocument.getDidId())
                        .fromStatus(didDocument.getDeactivated() ? DidDocStatus.DEACTIVATED : DidDocStatus.ACTIVATED)
