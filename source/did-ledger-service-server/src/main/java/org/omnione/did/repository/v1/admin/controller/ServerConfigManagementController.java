...
import org.omnione.did.repository.v1.admin.service.ServerConfigManagementService;
 ...
 * <p>
 * Provides endpoints for server configuration retrieval and updates.
 */
@Slf4j
...
public class ServerConfigManagementController {
    ...
    /**
     * Retrieves a specific server configuration by its key.
     *
     ...
     * Allows batch updating of server configuration values and descriptions.
     *
     * @param updateServerConfigReqDto the request data containing configurations to update
     * @return response containing update results and updated configurations
     */
    @ArticleLog(name = "[Server Config Manage] Update Server Config", description = "Updates multiple server configurations.",
            targetType = "SERVER_CONFIG", actionType = ActionType.UPDATE)
    @PutMapping(value = "/server-configs")
    public UpdateServerConfigResDto updateServerConfigs(@Valid @RequestBody UpdateServerConfigReqDto updateServerConfigReqDto) {
