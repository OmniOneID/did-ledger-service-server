...
import { fetchDidList } from '../../../apis/did-api';
...
const DidListPage = (props: Props) => {
    ...
    const fetchData = useCallback(async () => {
        ...
        try {
            ...
            console.error("Failed to fetch DID List ", err);
            navigate('/error', { state: { message: formatErrorMessage(err, "Failed to retrieve DID List") } });
        } finally {
            setLoading(false);
        }
    ...
    const getData = useCallback(async () => {
        ...
        try {
            ...
            await dialogs.open(CustomDialog, {
                title: 'Notification',
                message: formatErrorMessage(err, 'Failed to retrieve DID List'),
                isModal: true,
            });
        } finally {
            setLoading(false);
    ...
    return (
      <>
        ...
        <StyledContainer>
          <StyledSubTitle>DID List</StyledSubTitle>
          <CustomDataGrid 
              rows={rows} 
              columns={[
                  { 
                      ...
                      renderCell: (params) => (
                          <Link 
                              component="button"
                              variant='body2'
                              onClick={() => navigate(`/dids/did-list/${params.row.id}`)}
                  ...
                  { 
                      ...
                      align: 'center',
                      headerAlign: 'center',
                      renderCell: (params) => getRoleLabel(params.value)
                  },
                  { 
                      field: 'status', 
                      headerName: "Status", 
